...
int main(int argc, char **argv)
{
	...
	if (client == NULL)
	{
		fprintf (stderr, "error occurred in creating client.\n");
		return 1;
	}
	...
	if (peer == NULL)
	{
		fprintf (stderr, "error occurred in connection.\n");
		return 1;
	}
...
void game(char *missionname)
{
		...
		actorPos.actorID = 0;
		actorPos.x = actor[actorID].x;
		actorPos.y = actor[actorID].y;


		ENetPacket * packet = enet_packet_create(& actorPos, sizeof(ActorPosition),ENET_PACKET_FLAG_UNRELIABLE_FRAGMENT);
	...
	while (1)
	{
		while(enet_host_service (client, & event, 100) >0)
		{
			...
			case ENET_EVENT_TYPE_RECEIVE:
				...
				printf("Packet of length %u was received.\n",
				break;
