		...
		if (gameState.hosting == 0)
		{
			for (c = 0; c < gameState.actors; c++)
			{
        		if (aptr->type)
        		{
        			...
        			{
	        			case ACTOR_BOFH:
							if (aptr->playerID == gameState.myPlayerID)
							{
							playercontrol(aptr);
							}
					break;
        		...
        		if (aptr->type)
        		{
					...
					movegrenade(aptr);
						...
						if (bombs)
						{
			        		...
			        		if (!trapmsg)
			        		{
				        		if (finddist(aptr->x, aptr->y, gameState.localplayer->x, gameState.localplayer->y) < 40)
				        		{
			        				trapmsg = 1;
			        				printgamemsg("BEWARE THE TRAP!", 200);
			        			}
			        		}
			        		if (((gameState.localplayer->x & 0xfffff000) == (aptr->x & 0xfffff000)) &&
			        			((gameState.localplayer->y & 0xfffff000) == (aptr->y & 0xfffff000)))
			        		{
			        			if (gametime)
			        			{
			        				gametime = 0;
...
void drawactors(void)
{
        ...
        ACTOR *aptr;
        if (!gameState.actors) return;
        aptr = &gameState.actor[gameState.actors-1];
        for (c = 0; c < gameState.actors; c++)
        {
        	if (aptr->type)
        	{
				//if ((aptr == &gameState.actor[0]) || (checkvision(aptr)))
				if (aptr->type == ACTOR_BOFH || checkvision_client(aptr))
...
int checkvision(ACTOR *aptr)
{
	...
	for (c=0; c<gameState.actors; c++)
	{
		ACTOR *pptr = &gameState.actor[c];

		if (pptr->type != ACTOR_BOFH)
			continue;
		...
		if ((adx > 300*DEC) || (ady > 300*DEC)) continue;
		...
		if (adx > ady)
		{
			...
			if (dx > 0) dx = 16*DEC; else dx = -16*DEC;
				...
				if (sx != ex)
				{
					...
					if (blkinf & INF_BLOCKVISION) goto next_player;
				}
			}
		}
		else
...
int checkvision_client(ACTOR *aptr)
{

	ACTOR *pptr = gameState.localplayer;
    int sx, sy, ex, ey, dx, dy, adx, ady, blkinf;
...
int checkblockvision(int ex, int ey)
{
        ...
        sx = gameState.localplayer->x & 0xfffff000;
        sy = gameState.localplayer->y & 0xfffff000;
...
void playerattack(ACTOR *aptr)
{
        ...
        if ((!aptr->attackdelay) && ((win_keytable[KEY_SPACE]) || (mouseb & MOUSEB_LEFT)) && ammo[weapon]) // ***
	{
		switch(weapon)
		{
                	case WEAP_FISTS:
...
void breakglass(ACTOR *aptr, int x, int y)
{
	...
	if (blknum < 421)
	{
		...
		mapptr = (short*)&map_layerdataptr[1][map_layer[1].xsize*yb+xb];
		*mapptr += 420;
...
void structuredamage(int x, int y)
{
	...
	{
		mapptr = (short*)&map_layerdataptr[0][map_layer[0].xsize*yb+xb];
...
void moveitem(ACTOR *aptr)
{
		...
		/* Item stationary: check collision */
	  	radius = actorradius[gameState.localplayer->type];
	  	if (radius)
	  	{
	                int taken = 0;
	  		int xdist = abs((aptr->x - gameState.localplayer->x));
	  		int ydist = abs((aptr->y - gameState.localplayer->y));
	  		if ((xdist <= radius) && (ydist <= radius))
	  		{
	  			switch(aptr->type)
	  			{
	  				...
	  				case ACTOR_SMALLMEDIKIT:
	  				if (gameState.localplayer->health < 100)
	  				{
		  				gameState.localplayer->health += 10;
		  				if (gameState.localplayer->health > 100) gameState.localplayer->health = 100;
		  				taken = 1;
		  			}
	  				break;
	  				...
	  				if (gameState.localplayer->health < 100)
	  				{
		  				gameState.localplayer->health += 20;
		  				if (gameState.localplayer->health > 100) gameState.localplayer->health = 100;
		  				taken = 1;
		  			}
		  			break;
...
void punish(ACTOR *cptr, ACTOR *aptr, int damage)
{
        ...
        if ((cptr != &gameState.actor[0]) || (!lifecheat)) cptr->health -= damage;
 	...
 	if (cptr->health <= 0)
 	{
 		...
 		switch(cptr->type)
 		{
  			...
  			if (aptr->origin == &gameState.actor[0]) reward(enemyscore[cptr->type - ACTOR_FIRSTENEMY]);
...
//on disconnect
void clearplayeractor(int clearplayerID)
{
	ACTOR *aptr;
	int c;
	...
	for (c = 0; c <MAX_ACTOR; c++)
	{
		/*if (aptr->type == ACTOR_BOFH)
		{
			printf("BOFH for player id %d with actor id %d\n",aptr->playerID,aptr->actorID);
		}*/
		if (aptr->playerID==clearplayerID)
		{
			printf("removing BOFH with actorID %d for client %d\n",aptr->actorID,aptr->playerID);
			aptr->type = ACTOR_NONE;
			break;
		}
	aptr++;
...
ACTOR *spawnactor_internal(int type, int x, int y, int angle, int playerID)
{
	ACTOR *aptr = &gameState.actor[0];
	int c;

	if (!gameState.actors) gameState.actors = 1;
	for (c = 0; c < gameState.actors; c++)
	{
		if (!aptr->type)
		{
            ...
            aptr->origin = NULL;
			aptr->attackdelay = 0;
			aptr->playerID = playerID;
			aptr->actorID = nextactorID++;
			if(aptr->playerID > 0)
			{
				printf("spawning actor with actorid %d, and playerid %d\n",aptr->actorID,aptr->playerID);
			}
			return aptr;
		}
		aptr++;
        if ((c >= gameState.actors-1) && (gameState.actors < MAX_ACTOR)) gameState.actors++;
	}
	return NULL;
}

ACTOR *spawnreceivedactor(int type, int x, int y, int angle, int health, int frame, int attack,
						  void *origin, int enemymode, int enemycontrol, int enemycounter, int enemybored,
						  int special, int areanum, int actorID, int playerID)
{
	ACTOR *aptr = &gameState.actor[0];
	int c;
	...
	for (c = 0; c< gameState.actors; c++)
	{
		if (!aptr->type)
		{
			...
			aptr->attack = attack;
			aptr->origin = origin;
			aptr->enemymode = enemymode;
			aptr->enemycontrol = enemycontrol;
			aptr->enemycounter = enemycounter;
			aptr->enemybored = enemybored;
			aptr->special = special;
			aptr->areanum = areanum;
			aptr->actorID = actorID;
			aptr->playerID = playerID;
			//printf("spawned actor type %d with id %d\n",type,actorID);
			nextactorID++;
			return aptr;
		}
		aptr++;
...
{
	return spawnactor_internal(type, x, y, angle, 0);
}

ACTOR *spawnactor_player(int type, int x, int y, int angle, int playerID)
{
	return spawnactor_internal(type, x, y, angle, playerID);
} 
...
int findlocalplayeractorid(int playerid)
{
	//printf("myplayerid: %d, actorcount: %d, playerid: %d\n",gameState.myPlayerID, gameState.actors, playerid);
	int c;
	ACTOR *aptr = &gameState.actor[0];
	...
	//printf("ERROR: couldn't find localplayer\n");
	return NULL;
}
