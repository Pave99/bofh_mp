...
int myPlayerID;
int receivedotherplayerID;
ACTOR* OtherPlayerActor;
ACTOR* OtherActor;
ACTOR* SightedActor;
int once = 0;
int actorcount = 0;
int expectedactorcount = 0;
int localactorid = 0;


int main(int argc, char *argv[])
{
		int network_error = network_init();
		if (network_error != 0)
		{
...
int network_init()
{
	...
	{
		printf ("error occurred in initializing enet\n");
		return 1;
	}

	gameState.client = enet_host_create (NULL, 1, 2, 0, 0);
	if (gameState.client == NULL)
	{
		printf ("error occurred in creating client.\n");
		return 1;
	}
	...
	gameState.peer = enet_host_connect (gameState.client, & address, 2, 0);
	if (gameState.peer == NULL)
	{
		printf ("error occurred in connection.\n");
		return 1;
	}
	/*peer = enet_host_connect (gameState.client, & address, 2, 0);
	if (peer == NULL)
	{
		printf ("error occurred in connection.\n");
		return 1;
...
void showsplash(void)
{
        int handle;
        int delay = 0;
	char *splashbuf = (char*)malloc(64000);
	if (!splashbuf) return;
...
void game(char *missionname)
{
		...
		while(enet_host_service (gameState.client, & event, 10) >0)
			{
				switch (event.type)
						{
							...
							case ENET_EVENT_TYPE_RECEIVE:
								{
									...
									switch(type) {
										case MSG_PLAYER_ID:
										{
											...
											memcpy(&idMessage, event.packet->data, sizeof(PlayerIDMessage));
											
											//printf("sizeof recvd playeridmessage: %d\n",sizeof(idMessage));

											gameState.myPlayerID = idMessage.playerID;
											receivedotherplayerID = idMessage.otherPlayerID;
											printf("My playerid: %d\n",gameState.myPlayerID);
											if (receivedotherplayerID != gameState.myPlayerID)
											{
													printf("client %d is the host, using their actors\n",receivedotherplayerID);
													gameState.hosting = 0;
													clearactors();
													//telling server to send the actors
													ActorRequestMessage arm;
													arm.type = MSG_ACTOR_REQUEST;
													arm.playerID=gameState.myPlayerID;
													ENetPacket* armpacket = enet_packet_create(&arm, sizeof(ActorRequestMessage), ENET_PACKET_FLAG_RELIABLE);
													printf("sending actorrequest to the server\n");
													enet_peer_send(event.peer,1,armpacket);

															
													//telling server to send rest of the game data

													GameDataRequestMessage gdrm;
													gdrm.playerID = gameState.myPlayerID;
													gdrm.type = MSG_GAME_DATA_REQUEST;
													ENetPacket* gdrmpacket = enet_packet_create(&gdrm, sizeof(GameDataRequestMessage), ENET_PACKET_FLAG_RELIABLE);
													printf("sending gamedatarequest to the server\n");
													enet_peer_send(event.peer,1,gdrmpacket);
													break;
											}
											else
											{
													printf("no other players, hosting...\n");
													gameState.hosting = 1;
													//break;
											}
											break;
										...
										case MSG_ACTOR_DATA:
										{
											ActorData receivedActorData;
											memcpy(&receivedActorData, event.packet->data, sizeof(ActorData));
											expectedactorcount = receivedActorData.actorcount;
											ACTOR* recvactor = spawnreceivedactor(receivedActorData.type,receivedActorData.x,
											receivedActorData.y,receivedActorData.angle,receivedActorData.health,receivedActorData.frame,
											receivedActorData.attack,receivedActorData.origin,receivedActorData.enemymode,receivedActorData.enemycontrol,
											receivedActorData.enemycounter,receivedActorData.enemybored,receivedActorData.special,receivedActorData.areanum,
											receivedActorData.actorID,receivedActorData.playerID);
											if(receivedActorData.type == 1)
											{
												printf("placing actor type %d, with id: %d to x:%d,y:%d\n",receivedActorData.type,receivedActorData.actorID
																											,receivedActorData.x,receivedActorData.y);
											}
											actorcount++;
											if (actorcount == expectedactorcount) 
											{
												printf("received all %d actors from the server\n",expectedactorcount);
												printf("creating bofh with type %d, x=%d, y=%d, angle=%d, playerid=%d\n",ACTOR_BOFH, gameState.actor[0].x+100, gameState.actor[0].y+100, gameState.actor[0].angle, gameState.myPlayerID);

												//ACTOR *aptr;
												
												gameState.localplayer = spawnactor_player(ACTOR_BOFH, gameState.actor[0].x+100, gameState.actor[0].y+100, gameState.actor[0].angle, gameState.myPlayerID);
												//printf("1");
												gameState.localplayer->health=100;
												//printf("2");
												//gameState.localplayer;
												printf("creating bofh with actorid %d, pos=%d,%d\n",gameState.localplayer->actorID, gameState.localplayer->x, gameState.localplayer->y);
												//printf("jej");
											}
											break;
										}
										case MSG_GAME_DATA:
										{
											GameDataMessage receivedGameData;
											memcpy(&receivedGameData, event.packet->data, sizeof(GameDataMessage));
											bombs = receivedGameData.bombs;
											computers = receivedGameData.computers;
											terrorists = receivedGameData.terrorists;
											printf("bombs: %d, computers: %d, terrorists: %d\n",bombs,computers,terrorists);
											printf("received gamedata\n");
											break;
										}
									}
        ...
        if((!map_loadmap(mapnamebuf)) ||
           ...
           (!initgame(misnamebuf,gameState.myPlayerID, gameState.hosting)))
        {
			gfx_fillscreen(0);
        ...
        for (;;)
        {
			...
			if ((kbd_checkkey(KEY_ESC)) || (gameover > 400))
			{
				...
				printf("disconnected");
				enet_peer_disconnect(gameState.peer,gameState.myPlayerID);
				//enet_peer_disconnect(peer,gameState.myPlayerID);
				//clearplayeractor(gameState.localplayer->playerID);
				return;
			}
			...
			/* Draw map. Shifting for explosion when time runs out */
				...
				if (gameState.localplayer->type == ACTOR_BOFH) drawsight();
				drawscanner();
				/* Show closet? */
				drawnearcloset();
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if(gameState.hosting == 1)
		{
		//sending the game data once to the server
			for (once; once < 1; once++)
			{

				//chunk gamestate

				uint8_t compressedBuffer[COMPRESSED_GAMESTATE_MAX_SIZE];
				size_t compressedSize = compressGameState(gameState, compressedBuffer);
				uint8_t chunkBuffer[CHUNK_SIZE];
				size_t numChunks = (compressedSize + CHUNK_SIZE -1) / CHUNK_SIZE;

				printf("%d chunks are needed to send\n",numChunks);

				for (size_t i = 0; i< numChunks; ++i)
				{
					printf("sending chunk %d\n",i);
					size_t chunkStart = i * CHUNK_SIZE;
					size_t chunkLength = (CHUNK_SIZE < compressedSize - chunkStart) ? CHUNK_SIZE : (compressedSize - chunkStart);
					memcpy(chunkBuffer, compressedBuffer + chunkStart,chunkLength);

					GameStateChunk chunk;
					chunk.type = MSG_GAMESTATE_CHUNK;
					chunk.chunkID = i;
					chunk.totalChunks = numChunks;
					chunk.dataSize = chunkLength;
					memcpy(chunk.data, chunkBuffer, chunkLength);

					ENetPacket* chunkpacket = enet_packet_create(&chunk, sizeof(GameStateChunk), ENET_PACKET_FLAG_RELIABLE);
					enet_peer_send(gameState.peer,1,chunkpacket);
					enet_host_flush(gameState.client);
				}
				...
				//using channel 0 to minimize conflicts
				enet_peer_send(gameState.peer,1,gdmpacket);
				printf("sending gamedata to the server\n");	
				//enet_host_flush(gameState.client);

				//printf("actor 0 type=%d, playerid%d\n",gameState.actor[0].type,gameState.actor[0].playerID);
		...
		while(enet_host_service (gameState.client, &event, 100) >0)
			{
				switch (event.type)
				{
				case ENET_EVENT_TYPE_RECEIVE:
							...
							case MSG_PLAYER_POSITION:
								{
									...
									if (receivedPos.playerID != gameState.myPlayerID)
									{
										//printf("received pos %d,%d for client %d\n",receivedPos.x,receivedPos.y,receivedPos.playerID);

										for (int i=0; i<gameState.actors; i++)
										{
											if (gameState.actor[i].playerID == receivedPos.playerID)
											{
												OtherPlayerActor = &gameState.actor[i];
												//printf("found other player with id %d\n",OtherPlayerActor->actorID);
												break;
											}
										}
										...
										if (OtherPlayerActor)
										{
											OtherPlayerActor->x =		receivedPos.x;
											OtherPlayerActor->y =		receivedPos.y;
											OtherPlayerActor->angle =	receivedPos.angle;
											OtherPlayerActor->frame =	receivedPos.frame;
											OtherPlayerActor->attack=	receivedPos.attack;
											break;
										}
										else
										{
											OtherPlayerActor = spawnactor_player(ACTOR_BOFH, (int)receivedPos.x,(int)receivedPos.y,(int)receivedPos.angle,receivedPos.playerID);
											OtherPlayerActor->health=100;
											printf("spawned BOFH for playerid %d, with id %d\n",receivedPos.playerID,OtherPlayerActor->actorID);
											if (!OtherPlayerActor)
											{
												printf("error creating bofh for other player\n");
											}
							...
							case MSG_ACTOR_STATE:
								{
									...
									for (int i=0; i<gameState.actors; i++)
									{
										if (gameState.actor[i].actorID == recvdState.actorID)
										{
											OtherActor = &gameState.actor[i];
											//printf("ACTORPOS: found actor with id %d\n",OtherActor->actorID);
											break;
										}
									}
									...
									if (OtherActor)
									{
										OtherActor->x			= recvdState.x;
										OtherActor->y			= recvdState.y;
										OtherActor->angle		= recvdState.angle;
										OtherActor->frame		= recvdState.frame;
										OtherActor->attack		= recvdState.attack;
										OtherActor->enemybored	= recvdState.enemybored;
										OtherActor->enemycontrol= recvdState.enemycontrol;
										OtherActor->enemycounter= recvdState.enemycounter;
										OtherActor->enemymode	= recvdState.enemymode;
										break;
									}
								}
								break;
							case MSG_ACTOR_SIGHT:
								{
									...
									//received sighted actor packet

									for (int i=0; i<gameState.actors; i++)
									{
										if (gameState.actor[i].actorID == recvdacsimsg.actorID)
										{
											SightedActor = &gameState.actor[i];
											//printf("ACTORSIGHT: found actor with id %d\n",SightedActor->actorID);
											break;
										}
									...
									if (SightedActor)
									{
										//sending sighted actorposition to server
										ActorState actorstate;
										actorstate.actorID		= SightedActor->actorID;
										actorstate.x			= SightedActor->x;
										actorstate.y			= SightedActor->y;
										actorstate.angle		= SightedActor->angle;
										actorstate.frame		= SightedActor->frame;
										actorstate.attack		= SightedActor->attack;
										actorstate.enemybored	= SightedActor->enemybored;
										actorstate.enemycontrol	= SightedActor->enemycontrol;
										actorstate.enemycounter	= SightedActor->enemycounter;
										actorstate.enemymode	= SightedActor->enemymode;
										actorstate.type			= MSG_ACTOR_STATE;
										//printf("sending sighted actor %d to server\n",actorstate.actorID);
										ENetPacket* actorstatepacket = enet_packet_create(&actorstate, sizeof(ActorState), ENET_PACKET_FLAG_UNRELIABLE_FRAGMENT);
										enet_peer_send(gameState.peer,0,actorstatepacket);
									}
									break;
								}
							case MSG_DISCONNECT:
								{
									...
									memcpy(&disconnectedplayerid, event.packet->data, sizeof(int));
									printf("received client %d disconnection from the server\n",disconnectedplayerid);
									clearplayeractor(disconnectedplayerid);
									printf("removing disconnected BOFH");
								}
							break;
						}
					break;
		...
		if (gameState.hosting == 0)
		{
			ACTOR *aptr = &gameState.actor[0];
			int c;

			for (c = 0; c < gameState.actors; c++)
			{
				//if ((aptr->type == ACTOR_FISTMAN || aptr->type == ACTOR_PISTOLMAN || aptr->type == ACTOR_SADIST) && checkvision(aptr))
				if (aptr->type >=ACTOR_FIRSTENEMY && aptr->type <=ACTOR_LASTENEMY && checkvision_client(aptr))

				{
					//sending actorid of visible actors
					ActorSightMessage acsimsg;
					acsimsg.actorID		= aptr->actorID;
					acsimsg.playerID	= gameState.myPlayerID;
					acsimsg.type		= MSG_ACTOR_SIGHT;
					//printf("actor %d visible, sending data to server\n",acsimsg.actorID);
					ENetPacket* actorsightpacket = enet_packet_create(&acsimsg, sizeof(ActorSightMessage), ENET_PACKET_FLAG_RELIABLE);
					enet_peer_send(gameState.peer,1,actorsightpacket);
				}
				aptr++;
			}
		...
		if (gameState.hosting == 1)
		{
			...
			for (c = 0; c< gameState.actors; c++)
			{
				//if (aptr->type =! ACTOR_BOFH &&  checkvision(aptr))
				//if ((aptr->type == ACTOR_FISTMAN || aptr->type == ACTOR_PISTOLMAN || aptr->type == ACTOR_SADIST) && checkvision(aptr))
				if (aptr->type >=ACTOR_FIRSTENEMY && aptr->type <=ACTOR_LASTENEMY && checkvision(aptr))
				{
					//sending actorposition to server
					ActorState actorstate;
					actorstate.actorID		= aptr->actorID;
					actorstate.x			= aptr->x;
					actorstate.y			= aptr->y;
					actorstate.angle		= aptr->angle;
					actorstate.frame		= aptr->frame;
					actorstate.attack		= aptr->attack;
					actorstate.type			= MSG_ACTOR_STATE;
					actorstate.enemymode	= aptr->enemymode;
					actorstate.enemycontrol = aptr->enemycontrol;
					actorstate.enemycounter = aptr->enemycounter;
					actorstate.enemybored	= aptr->enemybored;
					actorstate.attackdelay	= aptr->attackdelay;

					//printf("sending moved actor %d to server\n",actorstate.actorID);
					ENetPacket* actorstatepacket = enet_packet_create(&actorstate, sizeof(ActorState), ENET_PACKET_FLAG_UNRELIABLE_FRAGMENT);
					enet_peer_send(gameState.peer,0,actorstatepacket);
				}
				aptr++;
		...
		//sending current playerposition to server
		PlayerPosition pos;
		pos.playerID =	gameState.localplayer->playerID;
		pos.x =			gameState.localplayer->x;
		pos.y =			gameState.localplayer->y;
		pos.angle =		gameState.localplayer->angle;
		pos.frame =		gameState.localplayer->frame;
		pos.attack =	gameState.localplayer->attack;
		pos.type =		MSG_PLAYER_POSITION;
		
		//printf("computers: %d\n",computers);
		//printf("My playerid:%d, actorid:%d\n",gameState.localplayer->playerID,gameState.localplayer->actorID);
		//printf("localactorid:%d\n",localactorid);
		//printf("type %d: player %d: Xpos: %d Ypos: %d\n",pos.type,pos.playerID,pos.x,pos.y);
		packet = enet_packet_create(&pos, sizeof(PlayerPosition), ENET_PACKET_FLAG_UNRELIABLE_FRAGMENT);
		enet_peer_send(gameState.peer,0,packet);
		//enet_host_flush(gameState.client);
	}
...
void drawscanner(void)
{
        ACTOR *aptr = &gameState.actor[0];
        int c, y;
		//int localactorid = findlocalplayeractorid(gameState.myPlayerID);
        ...
        for (c = 0; c < gameState.actors; c++)
        {
                ...
                switch(aptr->type)
                {
                        ...
                        case ACTOR_LEADER:
                        case ACTOR_SADIST:
                        xd = (aptr->x/DEC/8 - gameState.localplayer->x/DEC/8);
                        yd = (aptr->y/DEC/8 - gameState.localplayer->y/DEC/8);
...
void movelifts(void)
{
    ...
    int xb = gameState.localplayer->x >> 12;
    int yb = gameState.localplayer->y >> 12;
    int req = -1;
	liftnumber = 0;
	lifthere = 0;
	...
	if (gameover) return;
	if (gameState.localplayer->type != ACTOR_BOFH) return;
	...
	if (lifthere)
	{
        	...
        	req -= lift[atlift].firstfloor;
                ...
                if ((req >= 0) && (req < lift[atlift].floors) && (req != atliftfloor))
                {
                	...
                	gameState.localplayer->angle = lift[atlift].angle;
                	gameState.localplayer->x = (lift[atlift].liftfloor[req].xb << 12)+8*DEC;
                	gameState.localplayer->y = (lift[atlift].liftfloor[req].yb << 12)+8*DEC;
                	gameState.localplayer->x += sintable[gameState.localplayer->angle] * 10;
                	gameState.localplayer->y -= sintable[gameState.localplayer->angle+COS] * 10;
                        xpos = gameState.localplayer->x - 160*DEC;
                        ypos = gameState.localplayer->y - 100*DEC;
...
void findnearestfloor(int liftnum, int *x, int *y)
{
		...
		for (d = 0; d < lift[liftnum].floors; d++)
        {
        	...
        	tdist = finddist(gameState.localplayer->x, gameState.localplayer->y, tx, ty);
                if (tdist < mindist)
                {
                       	mindist = tdist;
                       	minx = tx;
...
void endpart(void)
{
        ...
        int hbonus = gameState.localplayer->health;
...
void drawsight(void)
{
		...
		xp = (gameState.localplayer->x & 0xffffff00) + 60 * sintable[gameState.localplayer->angle];
        yp = (gameState.localplayer->y & 0xffffff00) - 60 * sintable[gameState.localplayer->angle+COS];
        xp -= xshift;
        xp -= yshift;
        xp -= (xpos & 0xffffff00);
        yp -= (ypos & 0xffffff00);
        ...
        if (sightline)
        {
		sx = gameState.localplayer->x - (xpos & 0xffffff00);
		sy = gameState.localplayer->y - (ypos & 0xffffff00);
		dx = (xp - sx)/16;
		dy = (yp - sy)/16;
...
void printstatus(void)
{
	...
	sprintf(textbuf, "%03d", gameState.localplayer->health);
...
void doscroll(void)
{
	...
	int speedx = 0;
	int speedy = 0;
    xp = gameState.localplayer->x & 0xffffff00;
	//xp = aptr.x & 0xffffff00;
    yp = gameState.localplayer->y & 0xffffff00;
	//yp = gameState.localplayer.x & 0xffffff00;
    xp -= (xpos & 0xffffff00);
    yp -= (ypos & 0xffffff00);
...
int initstuff(void)
{
	...
	for (c = 0; c < sizeof(repeats)/sizeof(SAMPLELOOPPOINT); c++)
        ...
        firebuf = (unsigned char*)malloc(32641);
        if (!firebuf)
	{
...
void playpositionalfx(int x, int y, int chan, int fx, unsigned freq, int volume)
{
    ...
    channelextras[chan].vol = volume;
 	voladjust = finddist(gameState.localplayer->x, gameState.localplayer->y, x, y);
 	if (voladjust > 0)
 	{
 		voladjust /= 10;
 		if (voladjust > 56) voladjust = 56;
	 	angle = findangle(gameState.localplayer->x, gameState.localplayer->y, x, y);
	 	angle = angledist(gameState.actor[localactorid].angle, angle);
	 	angle &= 0x3ff,
...
void updatepositionalfx(int x, int y, int chan)
{
    ...
    if (!snd_sndinitted) return;
 	...
 	if (voladjust > 0)
 	{
 		...
 		if (voladjust > 56) voladjust = 56;
	 	angle = findangle(gameState.localplayer->x, gameState.localplayer->y, x, y);
	 	angle = angledist(gameState.localplayer->angle, angle);
	 	angle &= 0x3ff,
	 	panning = 128 + ((sintable[angle]*7)/16);
                volume -= voladjust;
	}
...
void playownedpositionalfx(ACTOR *aptr, int x, int y, int chan, int fx, unsigned freq, int volume)
{
    int panning, voladjust, angle, newvolume = 0;
 	voladjust = finddist(gameState.actor[localactorid].x, gameState.actor[localactorid].y, x, y);
 	if (voladjust > 0)
 	{
 		voladjust /= 10;
 		if (voladjust > 56) voladjust = 56;
