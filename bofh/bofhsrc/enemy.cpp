...
void grenadeattack(ACTOR *aptr)
{
	...
	int closestbofh = closestbofhactorid(aptr);
    a = findangle(aptr->x, aptr->y, gameState.actor[closestbofh].x, gameState.actor[closestbofh].y);
	ad = abs(angledist(aptr->angle, a));
    d = finddist(aptr->x, aptr->y, gameState.actor[closestbofh].x, gameState.actor[closestbofh].y);
...
void fistmanattack(ACTOR *aptr)
{
    ...
    if (finddist(aptr->x, aptr->y, gameState.actor[closestbofh].x, gameState.actor[closestbofh].y) <= 10)
    {
		if ((gameState.actor[0].health) && (!aptr->attackdelay) && ((rand()&63) >= fistpr[difficulty]))
		{
			aptr->attack = 9;
			playpositionalfx(aptr->x, aptr->y, FXCHAN_ENEMYSHOOT, SMP_SWISH, 22050, 64);
...
static int enemyveryfar(ACTOR *aptr)
{
	...
	int adx, ady;
        ...
        adx = abs(gameState.actor[closestbofh].x - aptr->x);
        ady = abs(gameState.actor[closestbofh].y - aptr->y);
	...
	if (!aptr->attack)
		...
		if (domove(aptr))
		{
                	...
                	if (!aptr->enemycounter)
                	{
                                ...
                                if (finddist(aptr->x, aptr->y, gameState.actor[closestbofh].x, gameState.actor[closestbofh].y) > 30)
                                {
			                aptr->enemycontrol = CB_FORWARD;
			                a = findangle(aptr->x, aptr->y, gameState.actor[closestbofh].x, gameState.actor[closestbofh].y);
	...
	if (!aptr->attack)
		...
		if (domove(aptr))
		{
                	...
                	if (!aptr->enemycounter)
                	{
                                ...
                                /* Hunt player */
			        if (finddist(aptr->x, aptr->y, gameState.actor[closestbofh].x, gameState.actor[closestbofh].y) > 20) aptr->enemycontrol = CB_FORWARD;
...
void pistolmanattack(ACTOR *aptr)
{
	...
	if ((!aptr->attackdelay) && (finddist(aptr->x, aptr->y, gameState.actor[closestbofh].x, gameState.actor[closestbofh].y) < PISTOL_MAXDIST) && ((rand() & 63) >= pistolpr[difficulty]))
        {
...
void shotgunmancontrol(ACTOR *aptr)
{
	...
	if (!aptr->attack)
		...
		if (domove(aptr))
		{
                	...
                	{
                                /* Hunt player */
			        if (finddist(aptr->x, aptr->y, gameState.actor[closestbofh].x, gameState.actor[closestbofh].y) > 30) aptr->enemycontrol = CB_FORWARD;
			        else aptr->enemycontrol = 0;
			       	a = findangle(aptr->x, aptr->y, gameState.actor[closestbofh].x, gameState.actor[closestbofh].y);
			        da = angledist(aptr->angle, a);
			        if (da < 0) aptr->enemycontrol |= CB_LEFT;
...
void shotgunmanattack(ACTOR *aptr)
{
	...
	if ((!aptr->attackdelay) && (finddist(aptr->x, aptr->y, gameState.actor[closestbofh].x, gameState.actor[closestbofh].y) < SHOTGUN_MAXDIST) && ((rand() & 63) >= shotgunpr[difficulty]))
        {
...
void uzimanattack(ACTOR *aptr)
{
	...
	if ((!aptr->attack) && (!aptr->attackdelay) && (finddist(aptr->x, aptr->y, gameState.actor[closestbofh].x, gameState.actor[closestbofh].y) < UZIMAN_MAXDIST) && ((rand() & 63) >= uzipr[difficulty]))
        {
...
void leaderattack(ACTOR *aptr)
{
	...
	if ((!aptr->attack) && (!aptr->attackdelay) && (finddist(aptr->x, aptr->y, gameState.actor[closestbofh].x, gameState.actor[closestbofh].y) < LEADER_MAXDIST) && ((rand() & 63) >= flamepr[difficulty]))
        {
	...
	if (!aptr->attack)
		...
		if (domove(aptr))
		{
				...
				if (!checkvision(aptr)) aptr->enemycontrol |= CB_FORWARD;
                                        ...
                                        da = angledist(gameState.actor[0].angle, a);
                                        dist = finddist(aptr->x, aptr->y, gameState.actor[closestbofh].x, gameState.actor[closestbofh].y);
...
void sadistattack(ACTOR *aptr)
{
	...
	if ((!aptr->attack) && (!aptr->attackdelay) && (finddist(aptr->x, aptr->y, gameState.actor[closestbofh].x, gameState.actor[closestbofh].y) < SADIST_MAXDIST) && ((rand() & 63) >= crossbowpr[difficulty]))
        {
                int delay = finddist(aptr->x, aptr->y, gameState.actor[closestbofh].x, gameState.actor[closestbofh].y)/2 - difficulty * 3;
...
void allpatrol(void)
{
        ...
        {
        	if ((gameState.actor[c].type >= ACTOR_FIRSTENEMY) && (gameState.actor[c].type <= ACTOR_LASTENEMY))
        	{
	        	if (gameState.actor[c].enemymode == MODE_ATTACK) gameState.actor[c].enemymode = MODE_PATROL;
	        }
        }
}
...
void detectplayer(ACTOR *aptr)
{
	...
	int a, ad;
        ...
        //if (gameState.actor[bofhactorid].type != ACTOR_BOFH) return;

	if (!checkvision(aptr)) return;
		printf("DETECT_VISION\n");
		int bofhactorid = closestbofhactorid(aptr);
        a = findangle(aptr->x, aptr->y, gameState.actor[bofhactorid].x, gameState.actor[bofhactorid].y);
        ad = angledist(aptr->angle, a);
	if (abs(ad) < 128)
	{
		printf("ATTACK\n");
...
void attackplayer(ACTOR *aptr)
{
	...
	if (gameState.actor[0].type != ACTOR_BOFH) return;
	aptr->enemymode = MODE_ATTACK;
...
int closestbofhactorid(ACTOR *aptr)
{
	int distarray[2];
	int idarray[2];
	...
	for (c = 0; c < gameState.actors; c++)
	{
		if (cptr->type == ACTOR_BOFH)
		{
			int bofhdist = finddist(aptr->x, aptr->y, cptr->x, cptr->y);
			distarray[d] = bofhdist;
			idarray[d] = cptr->actorID;
			if (bofhdist < 100)
			{
				//printf("bofh %d with distance %d \n",idarray[d],distarray[d]);
			}
			d++;
		}
	cptr++;
