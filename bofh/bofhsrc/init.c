...
int initgame(char *missionfilename, int actorID)
{
        int c;
        ACTOR *aptr;
        char *textptr;
	...
	for (;;)
	{
		...
		aptr = spawnactor_player(ACTOR_BOFH, bofhstartx, bofhstarty, bofhstartangle, gameState.myPlayerID);
        if (!aptr)
        {
        	closemf();
        	return 0;
