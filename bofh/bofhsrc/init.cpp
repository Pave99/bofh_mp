...
int initgame(char *missionfilename, int playerID, int hosting)
{
	int c;
    ACTOR *aptr;
    char *textptr;
    ...
    if (hosting == 1) {
		...
		clearactors();
		aptr = spawnactor_player(ACTOR_BOFH, bofhstartx, bofhstarty, bofhstartangle, playerID);
		if (!aptr)
		{
			closemf();
			printf("error creating bofh");
			return 0;
		}
		aptr->health = 100;
		/*printf("creating bofh for playerID %d\n",gameState.myPlayerID);

		printf("bofh health is now %d\n",gameState.localplayer->health);*/
		if(aptr->playerID == gameState.myPlayerID)
		{
			gameState.localplayer = aptr;
			printf("localplayer has id of %d playerid of %d\n",gameState.localplayer->actorID, gameState.localplayer->playerID);
		}
	}
